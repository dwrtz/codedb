...
fn collect_bundle_object_refs(kind: &str, payload: &JsonValue, refs: &mut Vec<String>) {
    match kind {
        ...
        "Expression" => {
            // Every hash-valued field of an expression payload references a
            // stored object (children, `type`/`*_type`, `type_args`, regions,
            // call symbols, static data) and `verify` enforces exactly that
            // invariant globally, so the generic extractor is complete by
            // construction. The old hand-maintained per-kind match here
            // silently ignored unknown kinds (#12): a `loop`/`return` or
            // vec/string program bundled WITHOUT those children and failed on
            // import with `bad_bundle_closure`.
            extract_hash_strings(payload, refs);
        }
        "FunctionDef" => {
